...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    console.log('RolesGuard canActivate',this.reflector.get<string>('roles', context.getHandler()));
    
    const requiredRole = this.reflector.get<string>('roles', context.getHandler()); // Retrieve the required role from metadata
    if (!requiredRole) {
