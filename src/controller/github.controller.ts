...
async callback(@Req() req: Request, @Res() res: Response) {
    ...
    if(userParsedfromCookie) {
        ...
        if(checkUser(userParsedfromGithub, req)?.message) return new Error(checkUser(userParsedfromGithub, req).message);
        const claim = await createClaim(userParsedfromGithub, this.userService, this.groupService);
        userParsedfromGithub.times.github_login = new Date();
        userParsedfromGithub.reqHeaders = req.headers;
        await this.userService.updatebySub(userParsedfromGithub.sub, userParsedfromGithub);
...
async disconnectGithub(@Req() req, @Res() res) {
    ...
    return res.json({ message: "Successfully disconnected github account.", user: user });
}
