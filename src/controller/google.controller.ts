...
export class GoogleController {
    ...
    async callback(@Req() req: Request, @Res() res: Response) {
        // This route is protected by Google authentication
        // NestJS will automatically redirect the user to Google for authentication
        const { user: { user: userParsedfromGoogle, profile } } = req.user as any;
        ...
        if(userParsedfromCookie) {
            ...
            const userRecord = await this.userService.findOne({ sub: userParsedfromCookie.sub });
            if(!userRecord) throw new Error("Couldn't find user record with sub:"+userParsedfromCookie.sub);
            ...
            if(checkUser(userParsedfromGoogle, req)?.message) return new Error(checkUser(userParsedfromGoogle, req).message);

            (checkUser(userParsedfromGoogle, req).message);
            const claim = await createClaim(userParsedfromGoogle, this.userService, this.groupService);
            userParsedfromGoogle.times.google_login = new Date();
            userParsedfromGoogle.reqHeaders = req.headers;
            await this.userService.updatebySub(userParsedfromGoogle.sub, userParsedfromGoogle);
    ...
    @Get('associate/:jwt')
    @UseGuards(GoogleOauthGuard)
    async associate(@Req() req: Request, @Res() res: Response, @Param('jwt') jwt: string) {
        try {
            const decodedToken = decodeJWT(jwt);
            if(!decodedToken) throw new Error("Invalid token");

            const user = await this.userService.findOne({ sub: decodedToken.sub });
            if(!user) throw new Error("Couldn't find user record with sub:"+decodedToken.sub);
    ...
    async disconnectGithub(@Req() req: Request, @Res() res: Response) {
        ...
        if(!user) throw new Error("Couldn't find user record with sub:"+userParsed.sub);
        user.ext.googleid = null;
        await this.userService.updatebySub(user.sub, user);
        return res.json({ message: "Successfully disconnected google account.", user: user });
    }
}